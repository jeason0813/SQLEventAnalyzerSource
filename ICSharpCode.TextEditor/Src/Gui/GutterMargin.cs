...
namespace ICSharpCode.TextEditor
{
	...
	public class GutterMargin : AbstractMargin, IDisposable
	{
		...
		public GutterMargin(TextArea textArea) : base(textArea)
		{
			...
			numberStringFormat.FormatFlags = StringFormatFlags.MeasureTrailingSpaces | StringFormatFlags.FitBlackBox | StringFormatFlags.NoWrap | StringFormatFlags.NoClip;
		}
		...
		public override void Paint(Graphics g, Rectangle rect)
		{
			...
			for (int y = 0; y < (DrawingPosition.Height + textArea.TextView.VisibleLineDrawingRemainder) / fontHeight + 1; ++y)
			{
				...
				if (rect.IntersectsWith(backgroundRectangle))
				{
					...
					{
						g.DrawString((curLine + 1).ToString(), lineNumberPainterColor.GetFont(TextEditorProperties.FontContainer), drawBrush, backgroundRectangle, numberStringFormat);
					}
				}
			}
		}
