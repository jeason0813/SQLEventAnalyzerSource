...
namespace ICSharpCode.TextEditor
{
	...
	public class TextArea : Control
	{
		...
		protected void RequestToolTip(Point mousePos)
		{
			...
			TextLocation logicPos = textView.GetLogicalPosition(mousePos.X - textView.DrawingPosition.Left, mousePos.Y - textView.DrawingPosition.Top); bool inDocument = textView.DrawingPosition.Contains(mousePos) && logicPos.Y >= 0 && logicPos.Y < Document.TotalNumberOfLines;
		...
		private void InvalidateLines(int lineBegin, int lineEnd)
		{
			...
			Rectangle r = new Rectangle(0, y - 1 - virtualTop.Y, Width, height + 3);
