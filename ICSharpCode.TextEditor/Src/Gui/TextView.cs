...
namespace ICSharpCode.TextEditor
{
	...
	public class TextView : AbstractMargin, IDisposable
	{
		...
		public int VisibleColumnCount
		{
			get
			{
				return DrawingPosition.Width / WideSpaceWidth - 1;
			}
		}
		...
		private int PaintLinePart(Graphics g, int lineNumber, int startColumn, int endColumn, Rectangle lineRectangle, int physicalXPos)
		{
			...
			{
				TextWord currentWord = currentLine.Words[wordIdx];
				...
				if (ColumnRange.WholeColumn.Equals(selectionRange) || (selectionRange.StartColumn <= currentWordOffset && selectionRange.EndColumn > currentWordEndOffset))
				{
				...
				else if (currentWord.Type == TextWordType.Tab)
				{
					...
					// go to next tabstop
					int physicalTabEnd = ((physicalXPos + MinTabWidth - lineRectangle.X) / WideSpaceWidth / TextEditorProperties.TabIndent) * WideSpaceWidth * TextEditorProperties.TabIndent + lineRectangle.X;
