...
namespace ICSharpCode.TextEditor.Document
{
	public class FoldingManager
	{
		...
		public List<FoldMarker> GetFoldingsFromPosition(int line, int column)
		{
			...
			if (foldMarker != null)
			{
				for (int i = 0; i < foldMarker.Count; ++i)
				{
					...
					if ((fm.StartLine == line && column > fm.StartColumn && !(fm.EndLine == line && column >= fm.EndColumn)) || (fm.EndLine == line && column < fm.EndColumn && !(fm.StartLine == line && column <= fm.StartColumn)) || (line > fm.StartLine && line < fm.EndLine))
					{
						foldings.Add(fm);
					}
				}
		...
		private List<FoldMarker> GetFoldingsByStartAfterColumn(int lineNumber, int column, bool forceFolded)
		{
			...
			if (foldMarker != null)
			{
				int index = foldMarker.BinarySearch(new FoldMarker(document, lineNumber, column, lineNumber, column), StartComparer.Instance);
