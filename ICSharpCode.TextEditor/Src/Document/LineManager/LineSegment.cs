	...
	public sealed class LineSegment : ISegment
	{
		...
		internal void InsertedLinePart(int startColumn, int length)
		{
			...
			if (anchors != null)
			{
				foreach (TextAnchor a in anchors)
				{
					if (a.MovementType == AnchorMovementType.BeforeInsertion ? a.ColumnNumber > startColumn : a.ColumnNumber >= startColumn)
					{
						a.ColumnNumber += length;
					}
				}
		...
		internal void SplitTo(LineSegment followingLine)
		{
			...
			if (anchors != null)
			{
				...
				foreach (TextAnchor a in anchors)
				{
					if (a.MovementType == AnchorMovementType.BeforeInsertion ? a.ColumnNumber > Length : a.ColumnNumber >= Length)
					{
